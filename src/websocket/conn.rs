...
use hyper::{header::{CONNECTION, HOST, SEC_WEBSOCKET_KEY, SEC_WEBSOCKET_VERSION, UPGRADE}, rt::Executor, upgrade::Upgraded, Request};
...
pub async fn connect_tls(host: &String, certificate: &String, cursor: Option<u64>)
    ...
    let tcp_stream = TcpStream::connect(&addr)
        .await.with_context(|| format!("Unable to open tcp connection to: {}", addr))?;
    ...
    let tls_stream = connector.connect(tls_domain, tcp_stream)
        .await.with_context(|| format!("Unable to establish tls connection to: {}", host))?;
    ...
    let (ws, _) =
        ...
        .await.with_context(|| format!("Unable to upgrade connection to websocket: {}", uri))?;
